...
function employeeMonthKey(idNumber, month) {
  return `${idNumber}__${month}`;
}
...
function makeMonthRows(month) {
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    rows.push({ date, startTime: '', endTime: '', note: '' });
  }
  return rows;
}
...
app.post('/api/auth/login', (req, res) => {
  ...
  if (!idNumber || idNumber.length < 5) {
    return res.status(400).json({ error: 'יש להזין תעודת זהות תקינה.' });
  }

  const db = readDb();
  if (!db.employees[idNumber]) {
  ...
  return res.json({
    employee: buildEmployeePayload(db.employees[idNumber])
  });
});
...
app.put('/api/employee/:idNumber', (req, res) => {
  const idNumber = sanitizeIdNumber(req.params.idNumber);
  const db = readDb();
  const employee = db.employees[idNumber] || {
    idNumber,
...
app.post('/api/employee/:idNumber/upload/:docType', upload.single('file'), (req, res) => {
  ...
  const allowed = ['idCard', 'taxCoordination'];
...
app.post('/api/timesheet/:idNumber/:month/submit', async (req, res) => {
  try {
    ...
    if (!timesheet) {
      return res.status(400).json({ error: 'לא קיימת טבלת שעות לחודש זה.' });
    }
    ...
    if (employee.documents?.idCard?.filename) {
      attachments.push({
        filename: employee.documents.idCard.originalName || employee.documents.idCard.filename,
        path: path.join(UPLOADS_DIR, employee.documents.idCard.filename)
      });
    }
    if (employee.documents?.taxCoordination?.filename) {
      attachments.push({
        filename: employee.documents.taxCoordination.originalName || employee.documents.taxCoordination.filename,
        path: path.join(UPLOADS_DIR, employee.documents.taxCoordination.filename)
      });
    }
...
function getMonthlySubmissions(db, month) {
  return db.submissions.filter((item) => item.month === month);
}
...
async function sendMonthlyDigest(month) {
  const db = readDb();
  if (db.monthlyDigestLog.some((item) => item.month === month)) {
    ...
    attachments: [
      {
        ...
        content: buildMonthlySummaryCsv(submissions, month),
        contentType: 'text/csv; charset=utf-8'
      }
    ]
  });

  db.monthlyDigestLog.push({
    month,
...
app.get('/api/admin/monthly-summary', async (req, res) => {
...
app.listen(PORT, () => {
  console.log(`Employee portal listening on http://localhost:${PORT}`);
});
